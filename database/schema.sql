...
-- Удалить потом
-- Обновляем тестовые данные с полными профилями
UPDATE users SET 
  age = CASE 
    WHEN id = '00000000-0000-0000-0000-000000000000' THEN 28
    WHEN id = '00000000-0000-0000-0000-000000000001' THEN 30
    WHEN id = '00000000-0000-0000-0000-000000000002' THEN 23
    WHEN id = '00000000-0000-0000-0000-000000000003' THEN 27
    WHEN id = '00000000-0000-0000-0000-000000000004' THEN 21
    WHEN id = '00000000-0000-0000-0000-000000000005' THEN 25
    WHEN id = '00000000-0000-0000-0000-000000000006' THEN 24
  END,
  gender = CASE 
    WHEN id IN ('00000000-0000-0000-0000-000000000000', '00000000-0000-0000-0000-000000000001', 
                '00000000-0000-0000-0000-000000000003', '00000000-0000-0000-0000-000000000005') THEN 'male'
    ELSE 'female'
  END,
  bio = CASE 
    WHEN id = '00000000-0000-0000-0000-000000000000' THEN 'Тестирую приложение для знакомств на мероприятиях. Люблю технологии и новые знакомства!'
    WHEN id = '00000000-0000-0000-0000-000000000001' THEN 'Люблю организовывать интересные мероприятия и знакомить людей'
    WHEN id = '00000000-0000-0000-0000-000000000002' THEN 'Люблю активный отдых и походы в горы. Ищу компанию для поездки на фестиваль!'
    WHEN id = '00000000-0000-0000-0000-000000000003' THEN 'Фанат джаза и крафтового пива. Буду рад новым знакомствам.'
    WHEN id = '00000000-0000-0000-0000-000000000004' THEN 'Учусь на дизайнера, обожаю выставки современного искусства.'
    WHEN id = '00000000-0000-0000-0000-000000000005' THEN 'Играю на гитаре, люблю настолки и квизы.'
    WHEN id = '00000000-0000-0000-0000-000000000006' THEN 'Веган, йога-инструктор. Ищу единомышленников.'
  END,
  interests = CASE 
    WHEN id = '00000000-0000-0000-0000-000000000000' THEN '["Технологии", "Музыка", "Путешествия", "Спорт"]'::jsonb
    WHEN id = '00000000-0000-0000-0000-000000000001' THEN '["Музыка", "Еда", "Культура"]'::jsonb
    WHEN id = '00000000-0000-0000-0000-000000000002' THEN '["Спорт", "Путешествия", "Музыка"]'::jsonb
    WHEN id = '00000000-0000-0000-0000-000000000003' THEN '["Музыка", "Еда", "Кино"]'::jsonb
    WHEN id = '00000000-0000-0000-0000-000000000004' THEN '["Культура", "Кино", "Прогулка"]'::jsonb
    WHEN id = '00000000-0000-0000-0000-000000000005' THEN '["Игры", "Музыка", "Спорт"]'::jsonb
    WHEN id = '00000000-0000-0000-0000-000000000006' THEN '["Спорт", "Еда", "Путешествия"]'::jsonb
  END
WHERE id IN (
  '00000000-0000-0000-0000-000000000000',
  '00000000-0000-0000-0000-000000000001',
